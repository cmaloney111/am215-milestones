...
def load_atp_data(filename='atp_matches_2024.csv'):
    ...
    print(f"  - Matches with tiebreaks: {df['had_tiebreak'].sum()} ({df['had_tiebreak'].mean()*100:.1f}%)")
    print(f"  - Match length distribution: {df['num_sets'].value_counts().sort_index().to_dict()}")
...
def compare_distributions(real_data, sim_results):
    ...
    print(f"  Real: mean={np.mean(real_durations):.1f}, std={np.std(real_durations):.1f}, median={np.median(real_durations):.1f}")
    print(f"  Sim:  mean={np.mean(sim_durations):.1f}, std={np.std(sim_durations):.1f}, median={np.median(sim_durations):.1f}")
    ...
    sim_sets_dist = pd.Series(sim_results['num_sets']).value_counts(normalize=True).sort_index()
...
def plot_comparisons(real_data, sim_results, lambda_s, lambda_r):
    ...
    ax = axes[0, 0]
    ax.hist(real_data['minutes'], bins=30, alpha=0.5, label='Real Data', density=True, color='blue')
    ax.hist(sim_results['durations'], bins=30, alpha=0.5, label='Simulation', density=True, color='red')
    ...
    ax = axes[0, 1]
    ax.hist(real_data['total_games'], bins=range(10, 50), alpha=0.5, label='Real Data', density=True, color='blue')
    ax.hist(sim_results['total_games'], bins=range(10, 50), alpha=0.5, label='Simulation', density=True, color='red')
    ...
    sim_sets = pd.Series(sim_results['num_sets']).value_counts(normalize=True).sort_index()
    ...
    width = 0.35
    ax.bar(x - width/2, real_sets_aligned.values, width, label='Real Data', color='blue', alpha=0.7)
    ax.bar(x + width/2, sim_sets_aligned.values, width, label='Simulation', color='red', alpha=0.7)
    ...
    sim_freqs = [sim_set_counts.get(s, 0) / len(sim_results['durations']) for s in all_scores]

    x_pos = np.arange(len(all_scores))
    width = 0.35
    ax.bar(x_pos - width/2, real_freqs, width, label='Real Data', color='blue', alpha=0.7)
    ax.bar(x_pos + width/2, sim_freqs, width, label='Simulation', color='red', alpha=0.7)
    ...
    sim_tb = [1 - np.mean(sim_results['had_tiebreak']), np.mean(sim_results['had_tiebreak'])]
    x_pos = np.arange(len(categories))
    width = 0.35
    ax.bar(x_pos - width/2, real_tb, width, label='Real Data', color='blue', alpha=0.7)
    ...
    plt.suptitle(f'Model Validation: λ_s={lambda_s:.5f}, λ_r={lambda_r:.5f}',
...
def grid_search_parameters(real_data, n_sims=500):
    ...
    for srv_adv in server_advantages:
        for total_rate in total_rates:
            ...
            duration_error = abs(np.mean(real_data['minutes']) - np.mean(sim_results['durations']))
            games_error = abs(np.mean(real_data['total_games']) - np.mean(sim_results['total_games']))
            tb_error = abs(real_data['had_tiebreak'].mean() - np.mean(sim_results['had_tiebreak'])) * 100

            # Weighted score (you can adjust these weights)
            score = duration_error * 0.5 + games_error * 2.0 + tb_error * 0.5

            print(f"Score={score:.2f} (dur_err={duration_error:.1f}, game_err={games_error:.1f}, tb_err={tb_error:.2f})")
...
if __name__ == "__main__":
    ...
    print(f"  Server advantage: {best_params[0]/(best_params[0]+best_params[1])*100:.1f}%")
    print(f"\nKey metrics:")
