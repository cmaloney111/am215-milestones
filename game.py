...
axes[0, 1].plot(
    times, prob_adv_r, "orange", linewidth=2, label="P(Ad-Receiver | Active)"
)
...
if idx < len(times):
    ...
    axes[1, 1].bar(
        states_labels, probs_snapshot, color=colors, alpha=0.7, edgecolor="black"
    )
    axes[1, 1].set_ylabel("Probability", fontsize=11)
    axes[1, 1].set_title(
        f"State Distribution at t={t_snapshot}s", fontsize=12, fontweight="bold"
    )
...
print(f"\nSome stats:")
print(
    f"- Peak deuce probability: {np.max(prob_deuce):.3f} at t={times[np.argmax(prob_deuce)]:.1f}s"
)
